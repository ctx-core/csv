...
export function csv__data_row_(csv, csv__parse_o) {
	const { header_M_row_idx, } = csv__parse_o.data
	...
	try {
		...
		for (const val_a of csv__parse(csv, csv__parse_o)) {
			return data_row_(val_a, header_M_row_idx)
		}
	} finally {
 ...
 * @private
 */
export function csv__data_row__parse_o_(header_M_row_idx) {
  const csv__parse_o = csv__parse_o_()
	csv__parse_o.data.header_M_row_idx = header_M_row_idx
	return csv__parse_o
}
